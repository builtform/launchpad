...
LaunchPad projects scaffolded via the v2.0 pipeline include `.mempalace/` in the gitignore by default, so users who choose to install MemPalace later have one less step. (v2.1 ships per-stack gitignore generation alongside lefthook generation per BL-245.)
