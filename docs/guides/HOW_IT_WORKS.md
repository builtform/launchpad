...
LaunchPad is an agentic coding harness with two main layers under the hood: a **governance kernel** plus an **installed plugin** that ride together inside any repository. The kernel is the persistent substrate (`REPOSITORY_STRUCTURE.md` whitelist, `lefthook.yml` pre-commit gates, `.launchpad/config.yml`, `.harness/` runtime, `docs/architecture/` core docs) that survives between sessions; the plugin is the 42 commands, 39 sub-agents, and 16 skills that operate against the kernel. The kernel lets each agent run inherit the previous run's findings instead of starting cold; the plugin gives Claude Code and Codex the verbs to brainstorm, define, plan, build, review, ship, and learn.
