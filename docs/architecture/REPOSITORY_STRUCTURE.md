...
- **One exception: a temporary review draft of a long document.** A proposed revision may sit beside its canonical file as `<name>.v2.md` while it is under review. For a file at the repository root, where the structure check rejects any extra file, the draft goes in `.harness/` instead. It is never committed (keep `*.v[0-9]*.md` in `.git/info/exclude`), and it is removed in the same step that folds it into the canonical file or discards it.
