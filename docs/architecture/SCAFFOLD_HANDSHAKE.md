...
### 10.v2.1.6 — v2.1.6 amendments (BL-345 + BL-347 + BL-349 + BL-350 + BL-346/BL-352 + BL-356)

Additive amendments to the v2.1 schema contract land in v2.1.6 from a post-v2.1.5 architectural scope review. The shapes below are additive — no v2.1.5 consumer breaks; rather, downstream renderers gain new sentinel-spliceable surfaces and `BackendInfo` gains one new required field.

- **BL-349: `BackendInfo.static_capable: bool` (required field).** `plugin_stack_adapters/contracts.py` `BackendInfo` TypedDict gains `static_capable: bool` as a required field. Every adapter's `describe_backend()` populates it. `static_capable=True` ⇒ `BACKEND_STRUCTURE.md` is rendered with "Static site — no backend" framing (omits Routes / Data models / Authentication / Error handling / Observability sections); `static_capable=False` ⇒ server-side sections render as v2.1.5. Astro / Eleventy / Expo / Hugo / Generic: `True`; FastAPI / Django / Rails / TS-monorepo / Go-CLI: `False`. Polyglot composition: `_merge_backend` prefers `static_capable=False` when frontend (static) + backend (server) layers are both present so the rendered BACKEND_STRUCTURE describes the actual server backend.

- **BL-347: stack-aware ALLOWED_DIRS / ALLOWED_CONFIGS injection.** `plugins/launchpad/scripts/plugin_stack_adapters/_structure_allowlists.py` declares per-stack additions for every stack id in `STACK_ID_ACTIVE_ENUM`. `lp_bootstrap/stack_structure_check.py` enricher reads `stacks:` from `.launchpad/config.yml`, looks up additions, and splices them between `STACK_AWARE_{DIRS,CONFIGS}_{BEGIN,END}` sentinel comments in the rendered `scripts/maintenance/check-repo-structure.sh`. The kernel template no longer hardcodes `apps/` + `packages/`; those move into the `ts_monorepo` stack entry. Greenfield + unknown-stack: fail-closed to kernel-only allowlist (strictly safer than v2.1.5's monorepo-hardcoded allowlist for any non-monorepo project).

- **BL-350: stack-aware `.gitignore` / `.gitleaks.toml` / `.greptile.json` patterns.** `plugins/launchpad/scripts/plugin_stack_adapters/_ignore_patterns.py` declares three per-stack dicts (`GITIGNORE_PATTERNS_PER_STACK`, `GITLEAKS_PATHS_PER_STACK`, `GREPTILE_IGNORE_PATTERNS_PER_STACK`). `lp_bootstrap/stack_ignore_patterns.py` enricher splices entries between `STACK_AWARE_GITIGNORE_{BEGIN,END}`, `STACK_AWARE_GITLEAKS_{BEGIN,END}` (in-array TOML comments), and `STACK_AWARE_GREPTILE_{BEGIN,END}` (in-string JSON sentinels — always consumed because they'd otherwise render as literal pattern values).

- **BL-346 + BL-352: stack-aware `lefthook.yml` + `.github/workflows/ci.yml` package-manager commands.** `plugins/launchpad/scripts/plugin_stack_adapters/_package_managers.py` declares `STACK_FAMILY` (per-stack-id → family) + `STACK_LEFTHOOK_HOOKS` (per-family command bodies). `lp_bootstrap/stack_pkg_manager.py` rewrites `run: pnpm <cmd>` body lines into family-appropriate equivalents for non-TS primary stacks. Families: `ts` (kernel default; byte-identical to v2.1.5), `ts_python` (composite nextjs_fastapi), `python` (pytest / ruff / pyright), `ruby` (bundle exec). v2.1.6 scope leaves the `pnpm/action-setup` + `actions/setup-node` setup-step blocks in ci.yml as TS-shape for non-TS projects (documented limitation; structural workflow rewrite deferred to v2.1.7).

- **BL-345: stack-detector extensions.** `plugin-stack-detector.py` `detect_from_manifest` recognises additional dependency markers — `astro` → stack id `astro`; single-app Next.js (no workspaces, no turbo) → stack id `nextjs_standalone`; non-Django Python (FastAPI / Flask / plain) → stack id `python_generic` (Django still → `python_django`). Surfaces `@11ty/eleventy`, `expo`, `@supabase/*` in the frameworks list as v2.2-active-enum candidates (no new stack id mapping at v2.1). `lp_define_runner._single_adapter` extended to dispatch `nextjs_standalone` and `python_generic` to the appropriate adapter — `nextjs_standalone` → the `NextjsStandaloneAdapter` Adapter Protocol class, `python_generic` → the `generic` module-level adapter shim (no Python-generic Adapter class exists yet; v2.2 candidate). The receipt schema is unchanged; the `stacks` array carries the new ids verbatim.

- **BL-356: `assert_autonomous_ack` raising-API helper + `AutonomousAckError` exception hierarchy.** `plugin_stack_adapters/autonomous_guard.py` gains `assert_autonomous_ack(repo_root) -> None` raising `AutonomousAckMissingError` if `.launchpad/autonomous-ack.md` is absent + `assert_ack_not_same_commit_as(repo_root, section_name) -> None` raising `AutonomousAckSameCommitError` (wraps the v2.1.5 `section_added_with_ack` predicate). The existing predicate is preserved for v2.1.5 callers; the raising APIs are additive. Refuse-message embeds the canonical starter template constant `AUTONOMOUS_ACK_TEMPLATE` so first-time users hitting the refuse have inline copy-paste content. Five command markdown files (`/lp-build`, `/lp-inf`, `/lp-resolve-todo-parallel`, `/lp-ship`, `/lp-plan`) reference the shared helpers by name; test-enforced single-source-of-truth invariant prevents inline absence-check drift.
...
v2.1.5 also renames `identity_inject` → `template_context` (the function now mixes identity surfacing with tool-version pins; the old name is preserved as a back-compat alias). Extended with two new context keys consumed by kernel + infrastructure templates: `default_pnpm_version` and `default_node_version`. Both source from `plugin_stack_adapters/_constants.py` (single source of truth). Bumping a constant there propagates to every rendered file that consumes the variable — the rendered CI workflow's `pnpm/action-setup` `version:` input + `.nvmrc` content. v2.1.6 BL-345 stack-aware refactor builds on these constants.

v2.1.5 also adds a structural assertion at render time: `assert_workflow_self_consistency(batch, cwd, *, error_cls)` in `infrastructure_renderer.py` (wrapping internal `_validate_workflow_self_consistency`) walks every rendered `.github/workflows/*.yml`, parses YAML, and asserts that any `node-version-file` input (extended in v2.1.6 BL-345 to also cover python/go/ruby) names a file present in the render batch (or already on disk under cwd). Mismatch raises `InfrastructureRenderError` and refuses the whole batch — defense-in-depth against the BL-353/BL-354-class regression at the next workflow-template rotation. Runs AFTER the secret-scan gate so error-message interpolation cannot leak secret-shaped content past the scanner refuse-all.
