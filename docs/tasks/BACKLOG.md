...
#### BL-332 - v2.1.5: `codex-review.yml` job lacks `timeout-minutes` (hung Codex API call burns up to 6h of runner time)

**Status (2026-05-15)**: NEW — surfaced during the v2.1.4 PR #67 ship. The Codex Code Review job (`.github/workflows/codex-review.yml` job `codex-review`) has no explicit `timeout-minutes:` setting and inherits GitHub Actions' 360-minute default. On 2026-05-15T06:55:23Z the `Run Codex Review` step (`openai/codex-action@e0fdf01...`) hung — same workflow + same prompt had completed 4× successfully on the same PR earlier the same day (last success at 06:05Z, all under 4 minutes). Hung run consumed ~6 hours of runner time before the maintainer manually cancelled it (`gh run cancel 25904829199`). The workflow's `continue-on-error: true` meant the hang did NOT block merge, but the `IN_PROGRESS` check rollup state lingered and the runner-time waste was real. Without a tightened timeout, a future Codex/OpenAI-side hang will repeat this 6-hour burn.

**Source**: `.github/workflows/codex-review.yml` job `codex-review` lacks `timeout-minutes:`. Cancel action: `gh run cancel <run-id>`.

**Driver**: every PR that triggers Codex review is exposed to the same 6-hour worst-case waste whenever the upstream Codex API has an availability event. Cost: paid runner minutes (Codex review is per-PR billed via `OPENAI_API_KEY`), maintainer attention to detect the hang, and check-rollup confusion.

**At v2.1.5 design time**: add `timeout-minutes: 10` to the `codex-review` job (under `runs-on: ubuntu-latest`). The historical max for the `Run Codex Review` step on this repo is ~3-4 minutes; 10 minutes is a comfortable ceiling that fails fast on hangs without false-positives on legitimately-long reviews. `continue-on-error: true` is already set, so a 10-minute timeout failure still won't block merge. Optional belt-and-suspenders: also add `timeout-minutes: 8` on the `Run Codex Review` step itself so the timeout fires INSIDE the action's exit handler rather than at the job-level kill (cleaner cancel semantics; the per-step cap MUST be lower than the job-level cap).

**Default decision**: defer to v2.1.5. Trivial 1-line YAML change. Out of scope for v2.1.4 lane (which is fix-the-three-ship-blockers). Tracked here so the next future-Codex-hang doesn't re-burn 6 hours.
