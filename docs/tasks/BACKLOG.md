...
5. **pytest-xdist `@pytest.mark.serial` for 2 perf tests.** `test_cold_fill_thousand_files_under_three_seconds` (3s budget) and `test_write_batch_perf_under_300ms_30file_scaffold` (300ms soft budget) fail under `-n auto` due to CPU contention. Phase 11 reconciliation #1 documents this; v2.1.0 ships verified via plain pytest. Add `@pytest.mark.serial` decorator + xdist `loadgroup` config so DoD runs cleanly under `-n auto`. Partially addressed 2026-08-03: `test_write_batch_perf_under_300ms_30file_scaffold` now warns at its 300ms soft target and fails only at a 1000ms hard limit, after a shared-runner slowdown failed a required check on an unrelated PR. `test_cold_fill_thousand_files_under_three_seconds` still carries a single 3s budget and remains exposed to the same contention.
...
**Status (2026-08-30)**: STILL DEFERRED, recurrence guarded. Dependabot raised the pair a third time (#155 `prisma` 6.19.3 -> 7.10.0, #158 `@prisma/client` 6.19.3 -> 7.10.0); both closed unmerged on 2026-08-30, matching the #54 and #86 outcomes. Root cause of the churn was a missing ignore rule: `.github/dependabot.yml` carried no Prisma entry, so every 7.x release re-raised two PRs that could never pass CI, each costing a manual triage pass. A `semver-major` ignore for `prisma` and `@prisma/client` now sits under the npm ecosystem block; 6.x minors and patches are deliberately still allowed so security fixes keep flowing. **Remove that ignore block as part of this BL.** Two corrections for whoever actions this: the target version is now 7.10.0, corrected in the **Files likely to modify** list below (the `7.8.0` in **Filed by** is deliberately left alone, since it records what PR #86 actually proposed), and the blast radius is smaller than the impact map implies. `packages/db` carries just two TypeScript sources (`src/client.ts` and `src/index.ts`) beside `package.json`, `tsconfig.json`, and `prisma/schema.prisma`; `prisma/migrations/` holds only a `.gitkeep`; the schema declares a single `User` model; and no source file in the repo imports `@repo/db` (only `apps/api/package.json` declares the dependency). The "breaks consumers in `apps/api` or `apps/web`" stop condition below therefore has no live callsite to bite; treat this as a template-correctness change rather than a data-layer migration. The v7 code snippets in this BL were re-verified against Prisma's current upgrade guide on 2026-08-30 and remain accurate.
...
- `packages/db/package.json`: bump `prisma` + `@prisma/client` to `^7.10.0` (latest 7.x as of 2026-08-30; re-check for a newer 7.x at implementation time and pin both to the same version); add `@prisma/adapter-pg` + `pg` + `@types/pg` deps; add `dotenv` if `prisma.config.ts` imports it.
...
- **Date**: 2026-09-19
- **Location**: `docs/architecture/SCAFFOLD_OPERATIONS.md` section 7.0a
- **Scenario**: Stable promotion intentionally skips the manual `workflow_dispatch` lane, but the canonical recovery text still prescribes that lane after a transient verification failure.

**Current Behavior**

The documented `gh workflow run v2-release.yml --ref <tag>` procedure can verify the tag successfully without running `promote-stable`, leaving marketplace users on the previous release.

**Desired Behavior**

The recovery contract tells maintainers to rerun the failed jobs of the original tag-push run with `gh run rerun <run-id> --failed` and states that the manual lane verifies only.

**Proposed Fix** (revalidate before implementing)

Update section 7.0a and any related canonical recovery references in a dedicated contract change. Preserve the existing contract review and validation requirements.
