...
/lp-harden-plan [plan-path] --auto          → Auto-apply (used by /lp-harness-plan)
...
`--interactive` is default when called from `/lp-harness-plan`.
...
IF `--interactive` (default when called from `/lp-harness-plan`):
...
### IF `--auto` (used by `/lp-harness-plan`):
