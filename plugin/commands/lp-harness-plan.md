...
3. IF status is `approved` or beyond → **REFUSE:** "Already approved. Run /lp-harness-build"
...
Refuse with descriptive error if inconsistent (e.g., "Status is 'approved' but approved_at field missing. Re-run /lp-harness-plan for human approval.").
...
| `defined` or no status           | "Not shaped. Run /lp-harness-define" |
