...
```
/lp-feature-video              → auto-detect routes from changed files
/lp-feature-video [url]        → record specific URL
/lp-feature-video --pr         → record and update PR description
```
...
1. `/lp-harness-plan` Step 2d — design walkthrough after design approval (optional)
2. `/lp-harness-build` after `/lp-ship` — feature demo for PR (optional)
