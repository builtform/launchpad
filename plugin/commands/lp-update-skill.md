...
```
/lp-update-skill creating-skills improve the extraction rounds
/lp-update-skill commit add Greptile review support
/lp-update-skill prd                    # prompts for change details
/lp-update-skill                        # prompts for skill name
```
...
- **Phase 6 (Validate):** Read `.claude/skills/creating-skills/references/QUALITY-GATES.md`. Spawn `lp-skill-evaluator` sub-agent (Sonnet) with access to BOTH old and new versions. Any gate that previously passed must still pass (regression check).
...
The skill must already exist in the catalog (added during `/lp-create-skill` or `/lp-port-skill`). Do NOT add new entries here — only update existing ones.
