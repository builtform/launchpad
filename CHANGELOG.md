...
- **Unpublished Codex compatibility candidate completed for branch review.** LaunchPad now has a projected Codex manifest, one bare `$lp` router skill, a sealed canonical package closure, deterministic support evidence, host-specific safety coordination, and isolated Codex/Claude lifecycle coverage. The candidate remains local dogfood: Codex CLI 0.153.4 did not authenticate bare `$lp` selection or provide a lossless authenticated argument tail, so all 44 public roots remain blocked and no workflow is advertised as executable.
- **Codex documentation and package identity are evidence-bound.** README and How It Works generated regions now render from qualification evidence, active guides distinguish the local plugin runtime from the Codex PR reviewer, and the completed package uses separate runtime, evidence, and detached artifact digest domains. No Codex marketplace listing, normal installation, tag, publication, or support activation is part of this candidate.
...
Every scaffolder recipe re-validated, CVE-watch lane repaired, dependency sweep. The 30-day freshness window on LaunchPad's scaffolder knowledge anchors lapsed while this release was being cut; rather than re-stamp 16 dates, every target was re-verified against live upstream sources. Zero of eleven anchors were accurate, and the executable catalog had drifted further still, breaking or degrading five stacks' scaffolds. v2.1.11 also clears the post-v2.1.10 Dependabot backlog (32 bumps, unblocked by the v2.1.10 freshness-gate fix) and repairs three CI failures that had been firing on a schedule with nobody acting on them: `cve-watch.yml` had been broken since `osv-scanner` v2 removed a flag it passed, that workflow's tag-drift detector was reporting false positives, and a routine `ruff` minor bump silently expanded the linter's default rule set under a config that could not pin it.

### For LaunchPad users (downstream behavior changes)

Scaffolding changes for five stacks (#145). Hono scaffolds failed at argument parsing on every invocation and now work. Astro scaffolds no longer install dependencies or run `git init/add/commit` mid-scaffold, both of which LaunchPad owns. Next, Astro and Expo scaffolds no longer overwrite the harness-authored `CLAUDE.md` (all three upstream CLIs began emitting agent instruction files by default), and Next output is now reproducible via `--reset-preferences`. Rails scaffolds gain the JavaScript entrypoint and Solid Queue/Cache/Cable config they were silently missing, at the cost of a longer scaffold step, because `--skip-bundle` suppressed the generators outright rather than deferring `bundle install`.

Eleven scaffolder knowledge anchors were rewritten for accuracy. Notably, the Eleventy anchor's claim that Eleventy 3 removed CommonJS was false and its `.mjs` rename pitfall would have broken a working repository.

Everything else is LaunchPad-internal. Downstream projects do not run `cve-watch.yml`, `v2-handshake-lint`, `anchor-freshness-watch.yml`, or the backlog-orphan gate. The BL-376 fix corrects documentation to match shipped behavior. The dependency bumps are build-time or template-scoped.

### Plugin-internal changes

- **All 16 freshness targets re-validated against live upstream (#145).** Three failure classes surfaced, and only the first is what a time-based window can catch: version drift (11 of 11 anchors); detection signals keyed on paths upstream has removed, which fail _silently_ by misidentifying a stack in a brownfield repo; and claims that were never true and so never drifted. Two anchors had been stamped `2026-06-23` while already contradicting upstream events from 2026-06-19 and 2026-06-22, direct evidence the prior re-stamp was a date bump rather than a check. Adapter and template layers were reconciled with what each actually materializes: `astro.py` stays on Astro 6, deliberately not the anchor's 7, because it renders templates pinned at `astro@6.2.2` rather than invoking the upstream CLI. New `test_adapter_version_agreement.py` (mutation-verified) asserts an adapter module and its own template fragments cannot disagree about a framework version.
- **Weekly anchor-freshness early warning (#144).** `anchor-freshness-watch.yml` runs `plugin-freshness-check.py` on a schedule and opens or updates a GitHub issue once any target enters a configurable lead window before its 30-day deadline, so staleness surfaces while it is cheap rather than as a blocked release. Split into two jobs so the checking job holds `contents: read` only and the `issues: write` job never checks out PR-controlled code. The verdict discriminator requires stdout and exit code to agree and errors loudly on any incoherent combination rather than defaulting to "fresh".
...
- **`vitest` 1.x to 4.x plus the first TypeScript tests (#120, #122, #124).** Migrated 1.6.1 to 3.2.6 (#120) to 4.1.10 (#122) and dropped a dead root workspace config (`vitest` 3 activates projects through `test.projects`). `vitest` 4 moves `vite` from a direct dependency to a peer dependency, so `vite` is now an explicit root dev dependency. #122 landed the first tests over `packages/shared` and `packages/ui`; #124 split the `apps/api` bootstrap into `app.ts` (app factory) and `index.ts` (server entry) so the app is testable without binding a port, and added the `/health` test.
...
See `docs/releases/v2.1.11.md` for the full dependency-bump table (9 major rows, 22 patch/minor rows; row count is below the 32 PR count because several rows consolidate more than one Dependabot PR).
...
- **BL-379** (P2): two OPERATIONS §4 mechanisms are documented but implemented by nothing, the re-stamp affirmation trailer and the `verdicts:` / `waivers:` report schema. The missing waiver path matters most: without one, the only thing that satisfies the release gate is editing the dates, which is exactly the pressure toward a dishonest stamp. Filed, deferred.
- **BL-380** (P3): promote the Supabase entry from `curate` back to `orchestrate`. The original demotion rationale is obsolete (`supabase init` is non-interactive now), but promotion needs its own verification pass against a real run, not a same-PR flip justified by reading docs.
- **BL-381** (P2): a per-stack post-install hook that would let Rails pass `--skip-bundle` again. Partially addressed in this release by removing the flag.
- **BL-382** (P3): adapter version metadata should be derived from `pin_registry.py` rather than hardcoded per module.
- **BL-383** (P2): no gate models cross-file claim propagation. One wrong sentence in an anchor reached five layers in this release and every copy was found by a reviewer, never by CI. The new adapter-agreement test covers one boundary; the rest are unguarded.
- **BL-365 / BL-366 / BL-367 / BL-368**: existing v2.1.x candidates, carrying forward.

### Test count

2011 passing + 4 skipped in the Python suite, measured on this release's head (up from 1975 + 4 at v2.1.9). The largest contributions are 12 tests from #138 (five pinning `--recover` semantics, seven pinning the freshness-lint UTC clock under shifted timezones) and 6 from #145. The TypeScript suite gained its first tests via #122 / #124.
