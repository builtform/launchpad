...
- **Conditional document review roster.** `.launchpad/agents.yml` gains an empty `review_document_agents` list. Projects that produce rendered pages, reports, PDFs, emails, dashboards, or generated documents can opt into `lp-document-truth` without binding document production to a technology stack.
