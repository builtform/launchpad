...
This project follows a PR-to-main workflow. Read this document before making changes — it defines the branching model, coding standards, testing requirements, and PR process.
...
11. [Safe Refactoring](#11-safe-refactoring)
...
For projects with a Python backend, also install Python 3.11+ and [poetry](https://python-poetry.org/).
...
git clone <repo-url>
cd <repo>
pnpm install          # installs all workspace deps + sets up lefthook git hooks
...
Formatting and linting are enforced automatically — do not manually fix style issues that tooling should handle.
...
All four must pass locally. CI re-runs the same suite in a clean environment and blocks merging on failure. Write tests for new behavior; place them next to the code they test or in the app's `tests/` directory.
...
- Do not place files at the repository root unless they are on the whitelist in Section 2 of REPOSITORY_STRUCTURE.md.
...
When uncertain, ask rather than guessing. CI enforces the structure on every PR.
...
2. Fill out the PR template in `.github/pull_request_template.md`. Include a summary of what changed and why, a link to the relevant issue, and manual test evidence using the VERIFIED/UNVERIFIED format (see Section 10).
