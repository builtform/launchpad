...
`plugin_default_generators/_renderer_base.py` adjacent to `template_context`
(the only consumer; was `identity_inject` pre-v2.1.5 C5 rename). KEPT as a
standalone module because v2.1.6 BL-345 stack-aware refactor adds
`DEFAULT_PYTHON_VERSION`, `DEFAULT_BUN_VERSION`, and `DEFAULT_RUBY_VERSION`
siblings — having a dedicated module avoids the "why are these constants
buried inside a renderer's helper file?" question when the stack-aware
refactor lands.
