...
Run `${CLAUDE_PLUGIN_ROOT}/scripts/plugin-prereq-check.sh --mode=lite --command=lp-create-agent --require=.launchpad/agents.yml`.

Verify-or-refuse — the lite helper checks the required file exists and exits 1 with a pointer to `/lp-define` if not; it does NOT create or seed missing files. Reads `config.yml` `paths.*` but primarily writes new agent definitions into `.claude/agents/` (the project-local user-agent location, separate from the plugin's built-in `${CLAUDE_PLUGIN_ROOT}/agents/`).
...
1. **Check if parameters were provided**:
   ...
   /lp-create-agent database migration auditor
   /lp-create-agent dependency checker from skill dependency-analysis
   /lp-create-agent                    # no args — prompts for details
   ```
...
3. **If no parameters provided**, respond with:
   ...
   **B) Convert an existing skill** — extract an agent from a skill's core competency
   ...
   Examples:
   - `/lp-create-agent security-reviewer for reviewing code security` (new agent)
   - `/lp-create-agent migration auditor from skill database-migrations` (convert skill)
   - `/lp-create-agent` (I'll walk you through it)
