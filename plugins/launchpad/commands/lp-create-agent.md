...
Create-if-missing only — the helper does NOT run the full detect/classify/present/scaffold protocol (that's harness-level). Reads `config.yml` `paths.*` but primarily writes into `.claude/agents/`.
