...
You are guiding the user through a deep-dive into a specific product section. This command creates a detailed section spec in `docs/tasks/sections/[section-name].md` and updates the section registry at `docs/tasks/SECTION_REGISTRY.md`.
...
- `docs/tasks/SECTION_REGISTRY.md` — **required** (the canonical section registry)
- `docs/architecture/PRD.md` — optional but useful (product purpose, users, non-goals)
...
**If `docs/tasks/SECTION_REGISTRY.md` is missing**, tell the user:

```
Section shaping requires a section registry at docs/tasks/SECTION_REGISTRY.md.
Run /lp-define to scaffold canonical state (PRD + section registry + tech stack
+ config.yml + agents.yml), then come back to /lp-shape-section.
```
...
2. Look up the normalized name in `docs/tasks/SECTION_REGISTRY.md` (use the `section_registry.load_sections(Path(repo_root))` helper from `${CLAUDE_PLUGIN_ROOT}/scripts/plugin_stack_adapters/section_registry.py`)
...
If the user chooses A, ask whether the new section is MVP or Post-MVP, then add it to `docs/tasks/SECTION_REGISTRY.md` with status `defined` before proceeding.

**If no arguments provided:**

1. Read `docs/tasks/SECTION_REGISTRY.md`
...
- If the section's status in `docs/tasks/SECTION_REGISTRY.md` is `planned`, `hardened`, `approved`, or `built`, warn the user: "This section is currently [status]. Re-shaping it will reset its status to `shaped`, which means any existing implementation plan may become stale." Ask them to confirm before proceeding.
...
- **From SECTION_REGISTRY.md:** current status, MVP flag, prior spec link (if any)
- **From PRD.md:** product purpose, users, non-goals, constraints (optional — enriches context)
...
After writing the section spec, update `docs/tasks/SECTION_REGISTRY.md` (the canonical registry):

1. Find the section entry (format: `### <section-name>` followed by `- **Status:** <status>`)
2. Update its status from `defined` to `shaped`
3. Ensure the spec link points at `sections/[section-name].md` (relative to the registry file)
4. Refresh the `- **Added:** <date>` / add a `- **Shaped:** <date>` line

If this was a newly added section (not previously in the registry), append a new entry block to the registry using the existing format.
...
Strategic skills produce blueprints that feed into web copy as upstream context. Skip silently if no strategic skills are installed.
...
**docs/tasks/SECTION_REGISTRY.md** registry updated: [section-name] → shaped
