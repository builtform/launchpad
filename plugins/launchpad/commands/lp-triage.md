...
```
/lp-triage              → triage all pending findings in .harness/todos/
```
...
| --------- | ------------------------------------------------------------ | --------------------------------- | ------------------------------------------------ |
| **fix**   | `status: pending` → `status: ready`                          | Stays in `.harness/todos/`        | `/lp-resolve-todo-parallel` picks it up          |
...
| **defer** | `status: pending` → `status: deferred`, adds `deferred_date` | Moved to `.harness/observations/` | Surfaces in backlog later                        |
...
- Fixed: {X} findings (ready for `/lp-resolve-todo-parallel`)
- Dropped: {Y} findings (moved to observations)
- Deferred: {Z} findings (moved to observations)

Run `/lp-regenerate-backlog` to update the project backlog with new deferrals.

Offer next steps: (1) Run `/lp-resolve-todo-parallel` to fix approved findings, (2) Done.
