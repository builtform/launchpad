...
If no sections are `shaped`, suggest running `/shape-section` first:
...
If the user chooses A, ask the relevant guided questions from `/shape-section` for just the missing items. If B, proceed but document assumptions clearly in the plan. If C, stop and suggest the command.
