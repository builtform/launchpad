...
```
/resolve-pr-comments                    → resolve comments on current branch's PR
/resolve-pr-comments 123                → resolve comments on PR #123
/resolve-pr-comments --auto-resolve     → also mark threads as resolved on GitHub
```
...
## Step 3: Spawn pr-comment-resolver Agents (max 5 concurrent, 5min timeout)
