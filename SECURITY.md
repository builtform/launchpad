...
5. **Commands-hash pin for CI / autonomous execution.** When `LP_CONFIG_REVIEWED` is set, the runner refuses any stage where the sha256 of the canonical `commands:` block in `.launchpad/config.yml` does not match the pinned value (full 64-char or 16-char prefix). This is the gate for CI and autonomous runs: pin the hash you reviewed, and any drift refuses. **Local interactive sessions trust the on-disk file** — when the env var is unset, the runner proceeds with whatever commands are in `config.yml`, on the assumption that an interactive operator would notice a malicious change in a `git diff` before invoking `/lp-commit`. Autonomous runs without the pin set should be considered out of scope for v1.0.0 — set `LP_CONFIG_REVIEWED` in CI, and review every PR that touches `config.yml` before merging.
...
- **Arbitrary commands in `.launchpad/config.yml`.** The runner executes every entry in `commands.test`, `commands.lint`, `commands.typecheck`, `commands.format`, and `commands.build` via `subprocess.run(cmd, shell=True)` so users can use shell features (pipes, glob expansion, `&&`-chaining). This is a deliberate ergonomics choice — config.yml commands like `pnpm test --filter=@apps/web` need shell parsing — and means that **whoever can write `config.yml` can run arbitrary shell at the agent's user level**. The mitigations in v1.0.0 are: (a) the `LP_CONFIG_REVIEWED` hash pin (item 5 above) for CI and autonomous runs, (b) PR review of any change to `config.yml` (call out `commands:` in your PR review checklist), (c) the recommended `dcg` companion below to catch destructive shell patterns regardless of source. An argv-array form of `commands:` with `shell=False` is on the v1.1 roadmap as opt-in for users who do not need shell features.
